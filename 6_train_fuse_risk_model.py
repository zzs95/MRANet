...
def call_optimization(model, max_epochs=None, warmup_epochs=None, learning_rate=None, learning_rate_start=None, learning_rate_end=None, weight_decay=None):
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=FACTOR_LR_SCHEDULER, patience=PATIENCE_LR_SCHEDULER, threshold=THRESHOLD_LR_SCHEDULER, cooldown=COOLDOWN_LR_SCHEDULER, verbose=True)
...
def main():
    ...
    for num_batch, batch in enumerate(test_loader):
        with torch.no_grad():
            # with torch.autocast(device_type="cuda", dtype=torch.float16):
            risks = model.risk_predict(batch, device)
